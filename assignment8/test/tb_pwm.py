...
tb_pwm.py V4.1 : Testbench for PWM module with fault injection. 
  By Yngve Hafting 2022, 2024, 2026 
        ...
        while True:
            ...
            try:
                ...
                if self.duty_mon.stable_interval('us') > interval:  
                    ...
                    set_duty = np.int8(np.int16(self.dut.duty_cycle.value.to_signed())*100/128) 
...
# FIAT tests checks that the testbench behaves as it should
@cocotb.test()
...
# Ordinary tests checks that the DUT behaves as it should. 
@cocotb.test()
async def ordinary_test_sequencer(dut):
    ''' Starts monitoring tasks and stimuli generators '''
    messages = MessageQueue()
