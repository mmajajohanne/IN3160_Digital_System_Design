...
architecture rtl of pulse_width_modulator is
  ...
  signal setpoint_i  : std_ulogic_vector(PDM_WIDTH-1 downto 0);
  signal pdm_pulse_i : std_ulogic;
  signal duty_abs    : signed(7 downto 0);
...
begin
  ...
  -- abs(duty_cycle) plasseres i MSB av 20-bit setpoint
  -- Mapper: 0 → 0%, 64 → ~50%, 127 → ~99.2%
  -----------------------------------------------------------------
  duty_abs <= abs(duty_signed);
  setpoint_i <= std_ulogic_vector(unsigned(duty_abs(6 downto 0))) & "0000000000000";
